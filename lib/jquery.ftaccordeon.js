...
(function($){
	$.fn.ftaccordion = function(options){
		...
		return this.each(function(){
			...
			$('.ftaccordion > .ac-wrapper:nth-child('+options.activeInset+')').addClass(options.activeClass).children('.ac-container').css('display','block');
			
			// parametr maxHeight
			if( options.maxHeight == true){
				var maxHeight = Math.max.apply(null, $('.ftaccordion .ac-container').map(function(){
					return $(this).height();
				}).get());
				$('.ftaccordion .ac-container').css('height',maxHeight);
			}
		});
	};
})(jQuery);